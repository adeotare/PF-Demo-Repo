...
import saveSiteMetadataRecords from "@salesforce/apex/P5InvertersBuildAndCounts.saveSiteMetadataRecords";
import getCustomDatatableCompInput from "@salesforce/apex/P5InvertersBuildAndCounts.getCustomDatatableCompInput";
import findNextPromptId from "@salesforce/apex/P5InvertersBuildAndCounts.findNextPromptId";
import findPreviousPromptId from "@salesforce/apex/Utils.findPreviousPromptId";
import clearPromptInfo from "@salesforce/apex/P5InvertersBuildAndCounts.clearPromptInfo";
...
import p5InverterNameIsBlankErrorMessage from '@salesforce/label/c.P5InverterNameIsBlankErrorMessage';
import p5SubArrayCountIsBlankErrorMessage from '@salesforce/label/c.P5SubArrayCountIsBlankErrorMessage';
import p5SubArrayandInverterNameIsBlankErrorMessage from '@salesforce/label/c.P5SubArrayandInverterNameIsBlankErrorMessage';
...
export default class P5InvertersBuildAndCounts extends LightningElement {
  @api plantAssetId;
  ...
  @api picklistValues;
  @track showImportAndExportButton;
  ...
  */
  ...
  validation() {
    let subArrayCountIsEmpty = false;
    let inverterNameIsEmpty = false;
    let isValid = true;
    ...
    this.siteMetadataIdToRecordDetails.forEach((value, key) => {
      ...
      JSON.parse(attrInfoJSON, (attrJSONKey, attrJSONValue) => {
        if (attrJSONKey === 'Sub Array') {
          let parsedAttrValue = parseInt(attrJSONValue, 10);
          if (parsedAttrValue <= 0 || (!parsedAttrValue)) {
            subArrayCountIsEmpty = true;
          }
        } else if (attrJSONKey === 'Inverter Name') {
          let parsedAttrName = attrJSONValue;
          if (isEmptyOrSpaces(attrJSONValue)) {
            inverterNameIsEmpty = true;
          }
    ...
    if(uniqueValueCheck){
      if (subArrayCountIsEmpty && inverterNameIsEmpty) {
        showAndHideSpinner(this, 'hidespinner');
        showToast(this, p5SubArrayandInverterNameIsBlankErrorMessage, 'Error', 'error', this.toastMessageMode);
        isValid = false;
      } else if (subArrayCountIsEmpty) {
        showAndHideSpinner(this, 'hidespinner');
        showToast(this, p5SubArrayCountIsBlankErrorMessage, 'Error', 'error', this.toastMessageMode);
        isValid = false;
      } else if (inverterNameIsEmpty) {
        showAndHideSpinner(this, 'hidespinner');
        showToast(this, p5InverterNameIsBlankErrorMessage, 'Error', 'error', this.toastMessageMode);
        isValid = false;
      }
    }else{
      isValid = false;
  ...
  handleClear(event) {
    if (event.detail.confirmationOutput) {
      ...
      clearPromptInfo({
        ...
        showAndHideSpinner(this, 'hidespinner');
        showToast(this, dataClearedErrorsMessage, 'error');
