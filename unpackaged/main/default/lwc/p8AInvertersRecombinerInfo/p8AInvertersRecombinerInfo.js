...
import getCustomDatatableCompInput from "@salesforce/apex/P8AInvertersRecombinerInfo.getCustomDatatableCompInput";
import clearPromptInfo from "@salesforce/apex/P8AInvertersRecombinerInfo.clearPromptInfo";
...
import findNextPromptId from "@salesforce/apex/P8AInvertersRecombinerInfo.findNextPromptId";
import saveSiteMetadataRecords from "@salesforce/apex/P8AInvertersRecombinerInfo.saveSiteMetadataRecords";
...
import p8ARecombinerAndSubArrayIsBlankMessage from '@salesforce/label/c.P8ARecombinerAndSubArrayIsBlankMessage';
import p8ARecombinerIsBlankMessage from '@salesforce/label/c.P8ARecombinerIsBlankMessage';
import p8ASubArrayIsBlankMessage from '@salesforce/label/c.P8ASubArrayIsBlankMessage';
...
export default class P8AInvertersRecombinerInfo extends LightningElement {
  @api plantAssetId;
  @api attrHlVal;
  ...
  */
  ...
  validation() {
    let recombinerNameIsEmpty = false;
    let subArrayIsEmpty = false;
    let isValid = true;
    ...
    this.siteMetadataIdToRecordDetails.forEach((value, key) => {
      ...
      JSON.parse(attrInfoJSON, (attrJSONKey, attrJSONValue) => {
        if (attrJSONKey === 'Recombiner Name') {
          if (isEmptyOrSpaces(attrJSONValue)) {
            recombinerNameIsEmpty = true;
          }
        ...
        } else if (attrJSONKey === 'Sub Array') {
          if (isEmptyOrSpaces(attrJSONValue)) {
            subArrayIsEmpty = true;
          }
        }
      });
    });
    ...
    if(uniqueValueCheck){
      if (recombinerNameIsEmpty && subArrayIsEmpty) {
        showAndHideSpinner(this, 'hidespinner');
        showToast(this, p8ARecombinerAndSubArrayIsBlankMessage, 'Error', 'error', this.toastMessageMode);
        isValid = false;
      } else if (recombinerNameIsEmpty) {
        showAndHideSpinner(this, 'hidespinner');
        showToast(this, p8ARecombinerIsBlankMessage, 'Error', 'error', this.toastMessageMode);
        isValid = false;
      } else if (subArrayIsEmpty) {
        showAndHideSpinner(this, 'hidespinner');
