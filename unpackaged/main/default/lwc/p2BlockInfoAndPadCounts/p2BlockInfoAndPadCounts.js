...
import getCustomDatatableCompInput from "@salesforce/apex/P2BlockInfoAndPadCounts.getCustomDatatableCompInput";
import saveSiteMetadataRecords from "@salesforce/apex/P2BlockInfoAndPadCounts.saveSiteMetadataRecords";
import findNextPromptId from "@salesforce/apex/P2BlockInfoAndPadCounts.findNextPromptId";
import findPreviousPromptId from "@salesforce/apex/Utils.findPreviousPromptId";
import clearPromptInfo from "@salesforce/apex/P2BlockInfoAndPadCounts.clearPromptInfo";
...
import p2PadCountandBlockNameValidationMessage from '@salesforce/label/c.p2PadCountandBlockNameValidationMessage';
import p2PadCountValidationMessage from '@salesforce/label/c.p2PadCountValidationMessage';
import p2BlockNameValidationMessage from '@salesforce/label/c.p2BlockNameValidationMessage';
...
export default class P2BlockInfoAndPadCounts extends LightningElement {
  @api plantAssetId;
  ...
  */	
  ...
  validation(){ 
    let padCountIsEmpty = false;
    let blockNameIsEmpty = false;
    var validCheck = true;
    ...
    this.siteMetadataIdToRecordDetails.forEach((value, key) => {
      ...
      let attrInfoJSON = siteMetadataRecordDetail.Prompt_Specific_Info__c;
        JSON.parse(attrInfoJSON, (attrJSONKey, attrJSONValue) => {
          if(attrJSONKey === 'Pad Count'){
            if(attrJSONValue.trim() === '' || attrJSONValue <= 0){
              padCountIsEmpty = true;
            }
          }else{
            if(attrJSONKey === 'Block Name'){
              if(attrJSONValue.trim() === ''){
                blockNameIsEmpty = true;
              }
    ...
    if(uniqueValueCheck){
      if(padCountIsEmpty && blockNameIsEmpty){
        showAndHideSpinner(this,'hidespinner');
        showToast(this,p2PadCountandBlockNameValidationMessage,'Error','error', this.toastMessageMode);
        validCheck = false;
      }else if(padCountIsEmpty){
        showAndHideSpinner(this,'hidespinner');
        showToast(this,p2PadCountValidationMessage,'Error','error', this.toastMessageMode);
        validCheck = false;
      }else if(blockNameIsEmpty){
        showAndHideSpinner(this,'hidespinner');
        showToast(this,p2BlockNameValidationMessage,'Error','error', this.toastMessageMode);
        validCheck = false;
      }
    }else{
      validCheck = false;
