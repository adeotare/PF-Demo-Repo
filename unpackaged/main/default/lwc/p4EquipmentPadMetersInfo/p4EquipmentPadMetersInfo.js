...
import saveSiteMetadataRecords from "@salesforce/apex/P4EquipmentPadMetersInfo.saveSiteMetadataRecords";
import getCustomDatatableCompInput from "@salesforce/apex/P4EquipmentPadMetersInfo.getCustomDatatableCompInput";
import findNextPromptId from "@salesforce/apex/P4EquipmentPadMetersInfo.findNextPromptId";
import findPreviousPromptId from "@salesforce/apex/Utils.findPreviousPromptId";
import clearPromptInfo from "@salesforce/apex/P4EquipmentPadMetersInfo.clearPromptInfo";
...
export default class P4EquipmentPadMetersInfo extends LightningElement {
  @api plantAssetId;
  @api previousPromptRecId;
  ...
  validation() {
    ...
    this.siteMetadataIdToRecordDetails.forEach((value, key) => {
      ...
      JSON.parse(attrInfoJSON, (attrJSONKey, attrJSONValue) => {
        if (attrJSONKey === 'Meter Name') {
          let parsedAttrValue = attrJSONValue;
          if (parsedAttrValue == '' || parsedAttrValue <= 0) {
            meterNameCheck = false;
          }
          
          //This is for Unique Name Validation Used For Bulk Import 
          let parsedAttrTrimValue = attrJSONValue.trim();
          if (!(isEmptyOrSpaces(parsedAttrTrimValue))) {
            if (!(allValues.includes(parsedAttrTrimValue))) {
              allValues.push(parsedAttrTrimValue);
            } else {
              uniqueValueCheck = false;
              showAndHideSpinner(this, 'hidespinner');
              showToast(this, uniqueNameValidation + '    ' + parsedAttrTrimValue, 'Error', 'error', this.toastMessageMode);
            }
          }
        }
      });
    ...
    if(uniqueValueCheck){
      if(!meterNameCheck){
        showAndHideSpinner(this, 'hidespinner');
        showToast(this, 'Meter Name should not be Empty', 'Error', 'error', this.toastMessageMode);
      }
      return meterNameCheck;
    }else{
      return false;
    }
  }
  ...
  handleClear(event) {
    if (event.detail.confirmationOutput) {
      ...
      clearPromptInfo({
        ...
        if (result !== 'false') {
          ...
          if(inputData.SiteMetadataRecords < this.buttonLabelsObj.recordCountForEachPrompt){
            querySiteMetadata({
              ...
              showAndHideSpinner(this, 'hidespinner');
              showToast(this, 'Data Cleared Successfully', 'Info', 'success');
            }).catch(error => {
              showAndHideSpinner(this, 'hidespinner');
            });
          }
        ...
        showAndHideSpinner(this, 'hidespinner');
        showToast(this, 'Data Not Cleared', 'Error', 'error');
      });
    } else {
      showAndHideSpinner(this, 'hidespinner');
      this.showConfirmPopupOnClear = false;
