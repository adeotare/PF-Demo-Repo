...
import getCustomDatatableCompInput from "@salesforce/apex/P6ModularInverterModuleInfo.getCustomDatatableCompInput";
import saveSiteMetadataRecords from "@salesforce/apex/P6ModularInverterModuleInfo.saveSiteMetadataRecords";
import findNextPromptId from "@salesforce/apex/P6ModularInverterModuleInfo.findNextPromptId";
...
import p6BValidationErrorMsgOnNext from '@salesforce/label/c.P6BValidationErrorMsgOnNext';
import p6BValidationErrorMsgOnSave from '@salesforce/label/c.P6BValidationErrorMsgOnSave';
...
import clearPromptInfo from "@salesforce/apex/P6ModularInverterModuleInfo.clearPromptInfo";
...
export default class p6ModularInverterModuleInfo extends LightningElement {
  @api plantAssetId;
  ...
  validation() {
    ...
    this.siteMetadataIdToRecordDetails.forEach((value, key) => {
      ...
      JSON.parse(attrInfoJSON, (attrJSONKey, attrJSONValue) => {
        if (attrJSONKey === 'Inverter Module Name') {
          if (attrJSONValue.trim() === '') {
            showToast(this, p6BValidationErrorMsgOnSave, 'Error', 'error', this.toastMessageMode);
            showAndHideSpinner(this, 'hidespinner');
            validCheck = false;
          }
    ...
    return (uniqueValueCheck) ? validCheck : false;
  }
