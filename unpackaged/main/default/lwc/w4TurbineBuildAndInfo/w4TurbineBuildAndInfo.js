...
import getCustomDatatableCompInput from "@salesforce/apex/W4TurbineBuildAndInfo.getCustomDatatableCompInput";
import saveSiteMetadataRecords from "@salesforce/apex/W4TurbineBuildAndInfo.saveSiteMetadataRecords";
import findNextPromptId from "@salesforce/apex/W4TurbineBuildAndInfo.findNextPromptId";
...
import clearPromptInfo from "@salesforce/apex/W4TurbineBuildAndInfo.clearPromptInfo";
...
import w4ValidationErrOnTurbineNameAndTurbineBuild from '@salesforce/label/c.W4ValidationErrOnTurbineNameAndTurbineBuild';
import w4ValidationErrOnTurbineName from '@salesforce/label/c.W4ValidationErrOnTurbineName';
import w4ValidationErrOnTurbineBuild from '@salesforce/label/c.W4ValidationErrOnTurbineBuild';
...
export default class W4TurbineBuildAndInfo extends LightningElement {
  ...
  */	
  ...
  validation() {
    var turbineNameCheck = true;
    var turbineBuildCheck = true;
    ...
    this.siteMetadataIdToRecordDetails.forEach((value, key) => {
      ...
      JSON.parse(attrInfoJSON, (attrJSONKey, attrJSONValue) => {
        if (attrJSONKey === 'Turbine Name') {
          if (attrJSONValue.trim() === '') {
            turbineNameCheck = false;
          }
        ...
        if (attrJSONKey === 'Turbine Build') {
          if ((attrJSONValue.trim() === '' || attrJSONValue.trim() === '--None--')) {
            turbineBuildCheck = false;
          }
        }
      });
    });
    ...
    if(uniqueValueCheck){
      if (turbineNameCheck && turbineBuildCheck) {
        showAndHideSpinner(this, 'hidespinner');
        return true;
      } else if (!turbineNameCheck && !turbineBuildCheck) {
        showAndHideSpinner(this, 'hidespinner');
        showToast(this, w4ValidationErrOnTurbineNameAndTurbineBuild, 'Error', 'error', this.toastMessageMode);
        return false;
      } else if (!turbineNameCheck) {
        showAndHideSpinner(this, 'hidespinner');
        showToast(this, w4ValidationErrOnTurbineName, 'Error', 'error', this.toastMessageMode);
        return false;
      } else if (!turbineBuildCheck) {
        showAndHideSpinner(this, 'hidespinner');
        showToast(this, w4ValidationErrOnTurbineBuild, 'Error', 'error', this.toastMessageMode);
        return false;
      }
    }else{
      return false;
