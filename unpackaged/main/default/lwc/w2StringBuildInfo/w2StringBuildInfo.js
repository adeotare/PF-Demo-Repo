...
import getCustomDatatableCompInput from "@salesforce/apex/W2StringBuildInfo.getCustomDatatableCompInput";
import saveSiteMetadataRecords from "@salesforce/apex/W2StringBuildInfo.saveSiteMetadataRecords";
import findNextPromptId from "@salesforce/apex/W2StringBuildInfo.findNextPromptId";
...
import clearPromptInfo from "@salesforce/apex/W2StringBuildInfo.clearPromptInfo";
...
import w2WindTurbineCountsIsBlankMessage from '@salesforce/label/c.W2WindTurbineCountsIsBlankMessage';
import w2StringNameIsBlankMessage from '@salesforce/label/c.W2StringNameIsBlankMessage';
import w2StringNameAndWindTurbineCountsIsBlankMessage from '@salesforce/label/c.W2StringNameAndWindTurbineCountsIsBlankMessage';
...
export default class W2StringBuildInfo extends LightningElement {
  ...
  */
  ...
  validation() {
    var validCheck = true;
    let stringNameIsEmpty = false;
    let windTurbineCountIsEmpty = false;
    ...
    this.siteMetadataIdToRecordDetails.forEach((value, key) => {
      ...
      JSON.parse(attrInfoJSON, (attrJSONKey, attrJSONValue) => {
        if (attrJSONKey === 'Wind Turbine Counts') {
          if ((isEmptyOrSpaces(attrJSONValue) || attrJSONValue === '0')) {
            windTurbineCountIsEmpty = true;
          }
        }
        if (attrJSONKey == 'String Name') {
          if (isEmptyOrSpaces(attrJSONValue)) {
            stringNameIsEmpty = true;
          }
    ...
    if(uniqueValueCheck){
      if (stringNameIsEmpty && windTurbineCountIsEmpty) {
        showAndHideSpinner(this, 'hidespinner');
        showToast(this, w2StringNameAndWindTurbineCountsIsBlankMessage, 'Error', 'error', this.toastMessageMode);
        validCheck = false;
      } else if (stringNameIsEmpty) {
        showAndHideSpinner(this, 'hidespinner');
        showToast(this, w2StringNameIsBlankMessage, 'Error', 'error', this.toastMessageMode);
        validCheck = false;
      } else if (windTurbineCountIsEmpty) {
        showAndHideSpinner(this, 'hidespinner');
        showToast(this, w2WindTurbineCountsIsBlankMessage, 'Error', 'error', this.toastMessageMode);
        validCheck = false;
      }  
    }else{
      validCheck = false;
