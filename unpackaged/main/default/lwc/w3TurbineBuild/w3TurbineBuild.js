...
import getCustomDatatableCompInput from "@salesforce/apex/W3TurbineBuild.getCustomDatatableCompInput";
import saveSiteMetadataRecords from "@salesforce/apex/W3TurbineBuild.saveSiteMetadataRecords";
import findNextPromptId from "@salesforce/apex/W3TurbineBuild.findNextPromptId";
...
import clearPromptInfo from "@salesforce/apex/W3TurbineBuild.clearPromptInfo";
...
import ValidationErrorMessage from '@salesforce/label/c.W3ValidationErrorMessage';
...
import attrCountAndBuildNameErrMsg from '@salesforce/label/c.AttrCountAndBuildNameErrMsg';
...
export default class W3TurbineBuild extends LightningElement {
  ...
  */
  ...
  validation() {
    ...
    if(uniqueValueCheck){
      if (validCheck && attrCountCheck) {
        return true;
      } else if (validCheck && !attrCountCheck) {
        showAndHideSpinner(this, 'hidespinner');
        showToast(this, attrCountValidationMessage, 'Error', 'error', this.toastMessageMode);
        return false;
      } else if (!validCheck && attrCountCheck) {
        showAndHideSpinner(this, 'hidespinner');
        showToast(this, ValidationErrorMessage, 'Error', 'error', this.toastMessageMode);
        return false;
      } else if (!validCheck && !attrCountCheck) {
        showAndHideSpinner(this, 'hidespinner');
        showToast(this, attrCountAndBuildNameErrMsg, 'Error', 'error', this.toastMessageMode);
        return false;
      }
    }else{
      return false;
